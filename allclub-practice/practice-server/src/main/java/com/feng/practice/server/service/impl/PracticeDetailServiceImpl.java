...
import com.feng.practice.server.rpc.UserRpc;
import com.feng.practice.server.service.PracticeDetailService;
import com.feng.practice.api.enums.*;
import com.feng.practice.api.req.*;
import com.feng.practice.api.vo.*;
import com.feng.practice.server.dao.*;
import com.feng.practice.server.entity.dto.*;
...
public class PracticeDetailServiceImpl implements PracticeDetailService {
    ...
    public Boolean submit(SubmitPracticeDetailReq req) {
        ...
        String second = timeUse.substring(4, 6); // 日期格式化 122051---12:20:51时分秒
        practicePO.setTimeUse(hour + ":" + minute + ":" + second);
        practicePO.setSubmitTime(DateUtils.parseStrToDate(req.getSubmitTime()));
        practicePO.setCompleteStatus(CompleteStatusEnum.COMPLETE.getCode()); //都点提交了，肯定状态完成，不管你做完没
        practicePO.setIsDeleted(IsDeletedFlagEnum.UN_DELETED.getCode());
        practicePO.setCreatedBy(LoginUtil.getLoginId()); //用户id，username，就是微信的openID
        practicePO.setCreatedTime(new Date());
        //计算正确率
        int correctCount = practiceDetailDao.selectCorrectCount(practiceId);
        List<PracticeSetDetailPO> practiceSetDetailPOS = practiceSetDetailDao.selectBySetId(setId); //这个套题的题目集合
        ...
        BigDecimal correctRate = new BigDecimal(correctCount).divide(new BigDecimal(totalCount), 4, RoundingMode.HALF_UP)
                .multiply(new BigDecimal("100.00")); // 正确数÷总数（保留四位小数）* 100.0
        practicePO.setCorrectRate(correctRate);
        PracticePO po = practiceDao.selectById(practiceId);
        if (Objects.isNull(po)) { // 该次练习没有练习过（意思就是上次练习没有中途退出）
            practiceDao.insert(practicePO);
        } else { //该次已经练习过了，由于某些原因中途暂停了，再次打开该练习
            practicePO.setId(practiceId);
            practiceDao.update(practicePO);
        }
        practiceSetDao.updateHeat(setId); // 提交一次，该套题练习热度加一点儿
        //补充剩余题目的记录（提交的时候，发现还有一些题目没有完成，即没有答过）
    ...
    public Boolean submitSubject(SubmitSubjectDetailReq req) {
        ...
        practicePO.setTimeUse(hour + ":" + minute + ":" + second);
        practicePO.setSubmitTime(new Date()); // 同上
        practiceDao.update(practicePO);
    ...
    public ReportVO getReport(GetReportReq req) {
        ...
        Long practiceId = req.getPracticeId();
        PracticePO practicePO = practiceDao.selectById(practiceId); // 获取这次练习
        Long setId = practicePO.getSetId();
        PracticeSetPO practiceSetPO = practiceSetDao.selectById(setId); // 获取这次练习的套题是哪个
        reportVO.setTitle(practiceSetPO.getSetName());
        List<PracticeDetailPO> practiceDetailPOList = practiceDetailDao.selectByPracticeId(practiceId); //获取这次练习的详细信息，一个practiceId对应多个subjectId
        if (CollectionUtils.isEmpty(practiceDetailPOList)) {
            return null;
        }
