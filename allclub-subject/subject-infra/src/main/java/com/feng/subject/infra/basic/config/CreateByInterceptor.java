package com.feng.subject.infra.basic.config;
...
public class CreateByInterceptor implements Interceptor {
    @Override
    public Object intercept(Invocation invocation) throws Throwable {
        MappedStatement mappedStatement = (MappedStatement) invocation.getArgs()[0];
        SqlCommandType sqlCommandType = mappedStatement.getSqlCommandType(); // sql语句的类型
        Object parameter = invocation.getArgs()[1];
        if (parameter == null) {
            return invocation.proceed();
        }
        //获取当前登录用户的id
        String loginId = LoginUtil.getUserId(); // 用户的openID，微信公众号验证码登录的
    ...
    private void replaceEntityProperty(Object parameter, String openId, SqlCommandType sqlCommandType) {
        if (parameter instanceof Map) {
            replaceMap((Map) parameter, openId, sqlCommandType);
        } else {
            replace(parameter, openId, sqlCommandType);
        }
    }
